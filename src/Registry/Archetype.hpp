...
namespace Astra
{
    ...
    /**
     * PackedLocation - Zero-cost abstraction for packed chunk/entity indices
     * 
     * Encodes both chunk index and entity index within chunk into a single size_t.
     * Enables O(1) lookups while maintaining cache locality.
     * 
     * Layout: [chunk index bits][entity index bits]
     * Example with 16-bit entity index: 0xCCCCCCCCCCCCEEEE
     */
    struct PackedLocation
    {
        ...
        constexpr PackedLocation() noexcept : value(std::numeric_limits<size_t>::max()) {}
        
        // Construct from raw value
        constexpr explicit PackedLocation(size_t v) noexcept : value(v) {}
        
        // Pack chunk and entity indices
        ASTRA_NODISCARD constexpr static PackedLocation Pack(size_t chunkIdx, size_t entityIdx, size_t shift) noexcept
        {
            return PackedLocation((chunkIdx << shift) | entityIdx);
        }
        
        // Extract chunk index
        ASTRA_NODISCARD constexpr size_t GetChunkIndex(size_t shift) const noexcept
        {
            return value >> shift;
        }
        
        // Extract entity index within chunk
        ASTRA_NODISCARD constexpr size_t GetEntityIndex(size_t mask) const noexcept
        {
            return value & mask;
        }
        ...
        ASTRA_NODISCARD constexpr size_t Raw() const noexcept { return value; }
        
        // Comparison operators for use in containers
        constexpr bool operator==(const PackedLocation& other) const noexcept { return value == other.value; }
        constexpr bool operator!=(const PackedLocation& other) const noexcept { return value != other.value; }
        constexpr bool operator<(const PackedLocation& other) const noexcept { return value < other.value; }
        constexpr bool operator>(const PackedLocation& other) const noexcept { return value > other.value; }
        constexpr bool operator<=(const PackedLocation& other) const noexcept { return value <= other.value; }
    ...
    class Archetype
    {
        static_assert(ChunkPool::CHUNK_SIZE == 16 * 1024, "Prefetching strategy assumes 16KB chunks");
    ...
    public:
        ...
        void Initialize(const std::vector<ComponentDescriptor>& componentDescriptors)
        {
            ...
            size_t remainingSpace = ChunkPool::CHUNK_SIZE - totalOffset;
        ...
        PackedLocation AddEntity(Entity entity)
        {
            ...
            if (chunkIdx >= m_chunks.size()) ASTRA_UNLIKELY
            {
                ...
                if (!chunk) ASTRA_UNLIKELY
                {
                    // Can't allocate new chunk - return invalid location
        ...
        */
        ...
        /**
         * Execute a function for each entity with specified components
         * 
         * Simple, clean iteration without software prefetching.
         * Modern CPUs handle sequential access patterns efficiently.
         */
        ...
        void ForEach(Func&& func)
        {
            ...
            for (auto& chunk : m_chunks)
            {
                ...
                // Simple iteration - let the CPU handle prefetching
                for (size_t i = 0; i < count; ++i)
                {
                    std::apply([&, i](auto*... ptrs) {
        ...
        void MoveEntityFrom(PackedLocation dstPackedLocation, Archetype& srcArchetype, PackedLocation srcPackedLocation)
        {
            ...
            for (const auto& dstComp : m_componentDescriptors)
            {
                void* dstPtr = m_chunks[dstChunkIdx]->GetComponentPointer(dstComp.id, dstEntityIdx);
                
                // Find matching component in source
                bool found = false;
                for (const auto& srcComp : srcArchetype.m_componentDescriptors)
                {
                    if (srcComp.id == dstComp.id) ASTRA_LIKELY
                    {
                        void* srcPtr = srcArchetype.m_chunks[srcChunkIdx]->GetComponentPointer(srcComp.id, srcEntityIdx);
                ...
                if (!found) ASTRA_UNLIKELY
                {
                    ...
                    dstComp.defaultConstruct(dstPtr);
                }
            }
        }
    ...
    private:
        ...
        public:
            ...
            static std::unique_ptr<Chunk, ChunkDeleter> Create(size_t entitiesPerChunk, const std::vector<ComponentDescriptor>& componentDescriptors, void* poolMemory = nullptr, ChunkPool* pool = nullptr)
            {
                ...
                size_t allocSize = ChunkPool::CHUNK_SIZE;
                bool usedHugePages = false;
                
                // Fall back to direct allocation if no pool memory provided
                if (!memory) ASTRA_UNLIKELY
                {
                    AllocResult result = AllocateMemory(ChunkPool::CHUNK_SIZE, 64);
                    memory = result.ptr;
                ...
                return std::unique_ptr<Chunk, ChunkDeleter>(new Chunk(entitiesPerChunk, componentDescriptors, memory, fromPool), deleter);
            }
        
            ~Chunk()
            {
                // Destruct all active components
                for (size_t i = 0; i < m_count; ++i)
                {
                    for (const auto& comp : m_componentDescriptors)
                    {
                        void* ptr = GetComponentPointer(comp.id, i);
                        if (ptr) ASTRA_LIKELY comp.destruct(ptr);
                    }
                }
            ...
            size_t AddEntity(Entity entity)
            {
                ...
                // Default construct components with optimization for POD types
                for (const auto& comp : m_componentDescriptors)
                {
                    if (comp.is_empty) ASTRA_UNLIKELY
                    {
                        // Empty types don't need initialization
                        continue;
                    }
                    
                    void* ptr = GetComponentPointer(comp.id, index);
                    
                    if (comp.is_trivially_copyable && comp.is_nothrow_default_constructible) ASTRA_LIKELY
                    {
                        // For POD types, skip initialization in release builds
                        #ifdef ASTRA_BUILD_DEBUG
                            std::memset(ptr, 0, comp.size);
                        #else
                            // Skip initialization for true POD types
                        #endif
                    }
                    else
                    {
                        // Non-POD types need constructor call
                        comp.defaultConstruct(ptr);
                    }
                }
            ...
            void BatchAddEntities(std::span<const Entity> entities)
            {
                ...
                // Batch default construct components
                for (const auto& comp : m_componentDescriptors)
                {
                    const auto& arrayInfo = m_componentArrays[comp.id];
                    if (!arrayInfo.base) ASTRA_UNLIKELY continue;

                    char* startPtr = static_cast<char*>(arrayInfo.base) + m_count * comp.size;

                    // Optimize based on component characteristics
                    if (comp.is_empty) ASTRA_UNLIKELY
                    {
                        // Empty types don't need any initialization
                        continue;
                    }
                    else if (comp.is_trivially_copyable && comp.is_nothrow_default_constructible)
                    {
                        // For POD types that are trivially default constructible,
                        // we can skip initialization entirely if they don't need zeroing
                        // This includes types like uninitialized floats/ints in release builds
                        
                        // In debug/safe mode, still zero for predictability
                        #ifdef ASTRA_BUILD_DEBUG
                            std::memset(startPtr, 0, count * comp.size);
                        #else
                            // Skip initialization for true POD types
                            // Components that need zero-initialization should have a constructor
                        #endif
                    }
                    else if (comp.is_trivially_copyable) ASTRA_UNLIKELY
                    {
                        // Trivially copyable but has constructor - zero initialize
                        std::memset(startPtr, 0, count * comp.size);
                    }
                    else
                    {
                        // Non-trivial types - must call constructor
                        for (size_t i = 0; i < count; ++i)
                        {
                            comp.defaultConstruct(startPtr + i * comp.size);
                        }
                    }
                }
            ...
            std::optional<Entity> RemoveEntity(size_t index)
            {
                ...
                if (index != lastIndex) ASTRA_LIKELY
                {
                    ...
                    for (const auto& comp : m_componentDescriptors)
                    {
                        void* dstPtr = GetComponentPointer(comp.id, index);
                        void* srcPtr = GetComponentPointer(comp.id, lastIndex);
                        
                        // Destruct destination, move from source
                        comp.destruct(dstPtr);
                        comp.moveConstruct(dstPtr, srcPtr);
                    }
                }
                else
                {
                    // Just destruct the last entity's components
                    for (const auto& comp : m_componentDescriptors)
                    {
                        void* ptr = GetComponentPointer(comp.id, lastIndex);
                        comp.destruct(ptr);
                    }
                }
            ...
            void* GetComponentPointerCached(size_t componentIndex, size_t entityIndex) const
            {
                ...
                return static_cast<char*>(m_arrayBases[componentIndex]) + entityIndex * m_componentDescriptors[componentIndex].size;
            }
            ...
            void* GetComponentPointer(ComponentID id, size_t index) const
            {
                ...
                return static_cast<char*>(info.base) + index * info.stride;
            }
            
        private:
            // Private constructor - only callable from factory
            Chunk(size_t entitiesPerChunk, const std::vector<ComponentDescriptor>& componentDescriptors, void* memory, bool fromPool)
                : m_memory(memory)
                ...
                , m_fromPool(fromPool)
            {
                ...
                // Clear memory
                std::memset(m_memory, 0, ChunkPool::CHUNK_SIZE);

                // Pre-compute array base pointers and strides
                m_arrayBases.resize(m_componentDescriptors.size());
                for (size_t i = 0; i < m_componentDescriptors.size(); ++i)
                {
                    void* arrayBase = static_cast<char*>(m_memory) + m_componentOffsets[m_componentDescriptors[i].id];
                    m_arrayBases[i] = arrayBase;
                    // Store both base pointer and stride for O(1) access by ComponentID
                    m_componentArrays[m_componentDescriptors[i].id] = {arrayBase, m_componentDescriptors[i].size};
                }
            }
            ...
            void CalculateLayout()
            {
                ...
                assert(offset <= ChunkPool::CHUNK_SIZE);
            }
        ...
        std::vector<std::pair<Entity, PackedLocation>> MoveEntitiesBetweenChunks(size_t srcChunkIdx, size_t destChunkIdx, size_t count)
        {
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                for (const auto& comp : m_componentDescriptors)
                {
                    void* srcPtr = srcChunk->GetComponentPointer(comp.id, srcEntityIdx);
                    void* destPtr = destChunk->GetComponentPointer(comp.id, destEntityIdx);
                    
                    // Use move constructor to transfer component data
                    comp.moveConstruct(destPtr, srcPtr);
                    comp.destruct(srcPtr);
                }
        ...
        static constexpr float COALESCE_UTILIZATION_THRESHOLD = 0.5f;  // Trigger when < 50% utilized
        static constexpr float SPARSE_CHUNK_RATIO_THRESHOLD = 0.25f;   // Trigger when > 25% chunks are sparse

        // ChunkPool reference (set by ArchetypeStorage)
        class ChunkPool* m_chunkPool = nullptr;

        friend class ArchetypeStorage;
        template<typename...> friend class View;  // For efficient iteration
